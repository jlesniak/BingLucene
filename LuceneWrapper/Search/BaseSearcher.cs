...
namespace LuceneWrapper.Search
{
    ...
    public abstract class BaseSearcher : BaseSearch
    {
        ...
        {
            Log.DebugFormat("Searching for Type: {0} with query \"{1}\" for field \"{2}\"", typeof (T), searchQuery,
                field);
            ...
            using (var searcher = new IndexSearcher(LuceneDirectory))
            {
                ...
                if (!string.IsNullOrEmpty(field))
                {
                    ...
                    {
                        throw new SearchException(string.Format("Field {0} is not a search field for type {1}", field,
                            typeof (T)));
                    }
                ...
                if (hits != null)
                {
                    ...
                    foreach (var hit in hits)
                    {
                        ...
                        {
                            Id =hit.Doc,// Convert.ToInt32(doc.Get("Id")),
                            Score = hit.Score,
                        });
                    }
                }
